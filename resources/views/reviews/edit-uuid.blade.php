<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Review (UUID Reference)</h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <p class="text-xs text-gray-500 mt-1">Current UUID used: <code>{{ $uuid }}</code></p>
                <p class="text-xs text-gray-500 mt-1">Sample UUID: <code>{{ $referenceSamples['uuid'] }}</code></p>
            </div>

            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('reviews.uuid.update', $uuid) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
